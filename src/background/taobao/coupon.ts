...
async function getDailygroupCoupon({
  ...
  const hasInventory = cacheHandler(async () => {
    var {
      data: { showBenefits },
    } = await getMeta();
    const firstItem = showBenefits[0];
    const b = firstItem.canWin === true || firstItem.canWin === "true";
    if (!b && (firstItem.hadWin === true || firstItem.hadWin === "true")) {
      throw new Error("抢过了");
    }
    return b;
  });
  ...
  showBenefits.forEach(async (item) => {
    if (item.increments && item.increments.length > 0) {
      ...
      taskManager
        ...
        .then(() => qiangquanToTime(t + 1000 * 60 * 3));
    }
    async function qiangquanToTime(t: number) {
      const res = await qiangquan().catch(() => ({ success: false }));
      const { success } = res;
      if (!success && Date.now() < t) {
        while (!(await hasInventory()) && Date.now() < t) {}
        return qiangquanToTime(t);
      }
    }
